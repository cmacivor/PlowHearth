...
def read_CC1CC2_PLC_PHFinalWeight(comm):
    ...
    ret = comm.Read("WXS_PHFinalWeight.TxTrigger", datatype=BOOL)
    ...
    if triggerBit == True:
        ...
        ret = comm.Read("WXS_PHFinalWeight.TxMessage.Code",datatype=STRING)
        ...
        ret = comm.Read("WXS_PHFinalWeight.TxMessage.Weight",datatype=REAL)
        TxWeight = ret.Value
...
while True:
    ...
    try:
        ...
        with PLC() as comm:
            ...
            PLC_Helpers.addPLCLog("CC1/CC2PH", "Final Weight", "Carton ID " + str(cc1cc2_phfinalweighttags["TxMessageCode"]))
            ...
            if cartonRecord is None:
                PLC_Helpers.addPLCLog("CC1/CC2PH", "No carton", "No carton record found in cartons table. cartonID: " + str(cc1cc2_phfinalweighttags["TxMessageCode"]))
                comm.Write("WXS_PHFinalWeight.TxTrigger", False)
                continue
            ...
            if weightRecord is None:
                PLC_Helpers.addPLCLog("CC1/CC2PH", "No weight", "No weight record found in weights table. cartonID: " + str(cc1cc2_phfinalweighttags["TxMessageCode"]))
                comm.Write("WXS_PHFinalWeight.TxTrigger", False)
                continue

            formattedWeight = PLC_Helpers.formatWeightActual(cc1cc2_phfinalweighttags["TxWeight"])
            ...
            #PLC_Helpers.updateWeight(cc1cc2_phfinalweighttags["TxMessageCode"], "CC1CC2", formattedWeight, "NA", "NA")
